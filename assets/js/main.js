...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initMediaThumbnails();
    initCuboAutoUpdate();
    initUnlockFeature(); // Adicionado para o desbloqueio por toque duplo
    initImageProtection();
    initTripleWakeLock();
});
...
/**
 * FUNÇÃO DE DESBLOQUEIO (UNLOCK)
 * Ativada por toque duplo no nome "Demian Max"
 */
function initUnlockFeature() {
    const artistaNome = document.querySelector('#sobre h2');
    if (artistaNome) {
        artistaNome.style.cursor = 'pointer';
        artistaNome.addEventListener('dblclick', () => {
            window.location.hash = 'unlock';
            console.log("🔓 Comando de desbloqueio ativado via toque duplo.");
        });
        
        // Suporte para toque duplo em dispositivos móveis
        let lastTap = 0;
        artistaNome.addEventListener('touchend', (e) => {
            const currentTime = new Date().getTime();
            const tapLength = currentTime - lastTap;
            if (tapLength < 500 && tapLength > 0) {
                window.location.hash = 'unlock';
                console.log("🔓 Comando de desbloqueio ativado via toque duplo (mobile).");
                e.preventDefault();
            }
            lastTap = currentTime;
        });
